...
%% 사용자 입력 단계
A[사용자 질문 입력<br/>Streamlit app.py] --> B[세션 저장 & UI 렌더링]

%% RAG 호출
B --> C[chatbot.ask(question)]

%% Retriever 단계
C --> D[Retriever 호출<br/>get_retriever()]
D --> E[ChromaDB 검색<br/>유사 문서 k개 반환]

%% 문서 포맷팅
E --> F[format_docs(docs)<br/>기업명 + 문장 결합]

%% 프롬프트 구성
F --> G[PromptTemplate 적용<br/>{context}, {question} 삽입]

%% LLM 단계
G --> H[Gemini-1.5-flash 호출]

%% 최종 응답
H --> I[답변 문자열 반환]
I --> J[Streamlit write_stream()<br/>Typing Animation 출력]
